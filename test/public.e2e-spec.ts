...
describe('Public Controller (e2e)', () => {
  ...
  it('Restaurant List GET 200', () => {
    return agent.get('/public/restaurant/list').expect(200);
  });

  it('Should return valid Restaurant list', async () => {
    return agent.get('/public/restaurant/list').then((response) => {
  ...
  it('Item List GET 200', () => {
    return agent
      .get(`/public/restaurant/${restaurants[0].id}/item/list`)
      .expect(200);
  });
  ...
  it('Should return valid Item list', () => {
    return agent
      ...
      .then((response) => {
        ...
        expect(new mongoose.Types.ObjectId(itemList[0].id)).toEqual(
          items[0].id,
        );
      });
  });
  ...
  it('Restaurant Registration POST 201', () => {
    return agent
      .post('/public/restaurant/register')
      .send(registerDto)
      .expect(201);
  });
  ...
  it('Search Restaurant GET 200', () => {
    return agent
      .get(`/public/restaurant/search?keyword=${searchKey}`)
      .expect(200);
  });
