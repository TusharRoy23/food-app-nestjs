...
export class OrderService implements IOrderService {
  constructor(
    ...
    @InjectModel(OrderItem.name, connectionName.MAIN_DB)
  ...
  async submitOrder(cartId: string): Promise<OrderResponse> {
    try {
      ...
      cart?.cart_items?.forEach((cartItem) => {
        const discountRate =
          cartItem.item?.discount_rate > 0
            ? cartItem.item.discount_rate / 100
            : 1;
      ...
      let rebate_amount = 0;
      let total_amount = orderAmount;
      const discountInfo = await this.sharedService.getOrderDiscount(
        cart.restaurant._id,
      );
      if (
        discountInfo?.discount_rate > 0 &&
        orderAmount <= discountInfo.max_amount &&
        orderAmount >= discountInfo.min_amount
      ) {
      ...
      await this.orderModel
        .findOneAndUpdate(
          { _id: orderInfo._id, user: user._id },
          { order_items: orderItems },
          { new: true },
        )
        .exec();

      await this.sharedService.updateCartInfo(
        { _id: cart._id, user: user._id, cart_status: CartStatus.SAVED },
